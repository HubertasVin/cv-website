...
const diffMode = ref<'words' | 'chars' | 'lines'>('words')
...
const diffResult = computed(() => {
  ...
  return Diff.diffWordsWithSpace(s1, s2)
})
...
<template>
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mb-8 mt-4 mx-auto max-w-4xl transition-colors"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
      <div>
        ...
        <textarea v-model="string1" rows="6"
          ...
          placeholder="Paste first string here..."></textarea>
      ...
      <div>
        ...
        <textarea v-model="string2" rows="6"
          class="w-full p-3 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow font-mono text-sm"
          placeholder="Paste second string here..."></textarea>
        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">Length: {{ string2.length }}</div>
      </div>
    </div>

    <div
      class="flex flex-wrap gap-4 mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-100 dark:border-gray-700">
      <label class="flex items-center space-x-2 cursor-pointer">
        <input type="checkbox" v-model="ignoreCase"
          class="rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500 w-4 h-4 bg-white dark:bg-gray-900">
    ...
    <div v-if="hasInput" class="rounded-lg overflow-hidden mb-6"
      :class="isMatch ? 'bg-green-100 dark:bg-green-900 border border-green-200 dark:border-green-800' : 'border border-gray-200 dark:border-gray-700'">
      <div v-if="isMatch"
        class="p-4 font-bold text-center text-lg transition-colors text-green-800 dark:text-green-100">
        Strings Match!
      </div>
      <div v-else>
        <div
          class="bg-gray-100 dark:bg-gray-800 px-4 py-2 border-b border-gray-200 dark:border-gray-700 font-semibold text-gray-700 dark:text-gray-300">
          Differences
        </div>
    ...
    <div v-if="!hasInput"
      class="rounded-lg p-4 text-center text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
